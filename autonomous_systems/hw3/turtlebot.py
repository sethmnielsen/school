...
class Turtlebot():
    ...
    def getMeasurements(self):
        ds = lmarks - self.state[0:2].reshape((2,1))
        r = np.sqrt(np.sum(ds**2, axis=0)) + np.random.normal(0, sig_r, size=(lmarks.shape[1]))
        theta = (np.arctan2(ds[1,:], ds[0,:]) - self.state[2]) + np.random.normal(0, sig_phi, size=(lmarks.shape[1]))
        theta = wrap(theta)
