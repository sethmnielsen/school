    ...
    def measure_accuracy(self, features, labels, confusion=None):
        ...
        if label_values_count == 0:
            ...
            for i in range(features.rows):
                ...
                # print('T: {:.2f}, P: {:.2f}'.format(targ[0],pred[0][0]))
                delta = targ[0] - pred[0][0]
                sse += delta**2
            ...
            prediction = []
            for i in range(features.rows):
                if i % 500 == 0 or i == features.rows-1:
                    t2 = time.time()-t1
                    # print('Instance: {}/{}, Time: {:.2f}s'.format(i,features.rows,t2))
                feat = features.row(i)
