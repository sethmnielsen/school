...
class plotData:
    ...
    def __init__(self):
        ...
        self.th_history = []  # angle
        self.th_ref_history = []
        self.volt_history = []  # control volt

        # create a handle for every subplot.
        self.handle = []
        self.handle.append(myPlot(self.ax[0], ylabel='Angle (deg)', title='DC Motor SFC'))
    ...
    def updatePlots(self, t, reference, states, ctrl):
        ...
        self.th_history.append(states[0]*180.0/np.pi)  # rod angle (converted to degrees)
        self.th_ref_history.append(reference[0]*180.0/np.pi)  # reference base position
        self.volt_history.append(ctrl[0])  # voltage

        # update the plots with associated histories
        self.handle[0].updatePlot(self.time_history, [self.th_history, self.th_ref_history])
        self.handle[1].updatePlot(self.time_history, [self.volt_history])
