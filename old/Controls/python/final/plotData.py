...
class plotData:
    ...
    def __init__(self):
        ...
        self.x3_history = []  # angle z
        self.force_history = []  # control force

        # create a handle for every subplot.
        self.handle = []
        self.handle.append(myPlot(self.ax[0], ylabel='p2 - p1 (m)', title='Double Sping-Mass-Damper Data'))
    ...
    def updatePlots(self, t, reference, states, ctrl):
        ...
        self.ref_history.append(reference[0])  # reference base position
        self.x3_history.append(states[2])  # rod angle (converted to degrees)
        self.force_history.append(ctrl[0])  # force on the base

        # update the plots with associated histories
        self.handle[0].updatePlot(self.time_history, [self.x3_history, self.ref_history])
